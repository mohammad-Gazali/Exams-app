...
from exams.models import Exam, MultipleChoiceQuestion, TrueFalseQuestion, EssayQuestion, ChoiceAnswer, EssayQuestionKeyword, TakingExamSession
from exams.forms import ExamForm, MultipleChoiceQuestionForm, ChoiceAnswerForm, TrueFalseQuestionForm, EssayQuestionForm
...
def finish_exam(request: HttpRequest, exam_id: UUID) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if exam.mcq_questions.all().count() + exam.true_false_questions.all().count() + exam.essay_questions.all().count() == 0:
            problems.append("zero questions")
            return render(request, "teacher/exams/handle_finish_exam_problem.html", {"problems": problems})
        ...
        if problems:
            return render(request, "teacher/exams/handle_finish_exam_problem.html", {"problems": problems, "extra_info": extra_info})
        else:
...
def exam_details(request: HttpRequest, exam_id: UUID) -> HttpResponse:
    exam = get_object_or_404(Exam.objects.prefetch_related("mcq_questions", "true_false_questions", "essay_questions"), id=exam_id)
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)
...
def take_exam(request: HttpRequest, exam_id: UUID) -> HttpResponse:
    ...
    if (current_teacher == exam.teacher) or (request.user.is_superuser) or (current_normal_user in exam.students.all()):  # type: ignore
        if not exam.is_finish:
            return HttpResponseBadRequest("<h1>400<h1/><h1>This Exam Is Not Finish</h1>")
        ...
        if request.method == "POST":
            ...
            if start_time is None:
                return HttpResponseBadRequest("<h1>400<h1/><h1>Start time is required</h1>")
            ...
            for key, answer in request.POST.items():
                ...
                if question_type == "mcq":
                    ...
                    # check if the question is in the exam's questions
                    if question := exam.true_false_questions.filter(id=question_id).first():
                    ...
                    if question := exam.essay_questions.filter(id=question_id).first():
                        user_answer_list = list(map(lambda x: x.lower(), format_right_answer_essay(answer)))
                        if get_language() == "en":
                            keywords_list = question.essayquestionkeyword_set.all().values_list('content_english', flat=True)
                            keywords_list = list(map(lambda x: x.lower(), keywords_list))
                        else:
                            keywords_list = question.essayquestionkeyword_set.all().values_list('content_arabic', flat=True)
                            keywords_list = list(map(lambda x: x.lower(), keywords_list))
        ...
        return render(request, "exams/take_exam.html", {
            "exam": exam,
            "start_time": timezone.datetime.now().timestamp()
        })
    else:
        return HttpResponseForbidden("<h1>403<h1/><h1>Forbidden</h1>")
...
@user_passes_test(teacher_user_test, "teacher")
def create_question(request: HttpRequest, q_type: Literal["mcq", "true-false", "essay"]) -> HttpResponse:
    ...
    return render(request, "teacher/exams/create_question.html", {
        "form": form,
        "type": q_type
    })
...
@user_passes_test(teacher_user_test, "teacher")
def add_questions(request: HttpRequest, exam_id: UUID, q_type: Literal["mcq", "true-false", "essay"]) -> HttpResponse:

    # exam and user data
    exam = get_object_or_404(Exam.objects.prefetch_related("mcq_questions", "true_false_questions", "essay_questions"), id=exam_id)
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)
    questions_number = exam.mcq_questions.all().count() + exam.true_false_questions.all().count() + exam.essay_questions.all().count()
...
def add_questions_created_before(request: HttpRequest, exam_id: UUID) -> HttpResponse:
    ...
    before_mcq_questions_ids = [question["id"] for question in exam.mcq_questions.all().values("id")]
    before_true_false_questions_ids = [question["id"] for question in exam.true_false_questions.all().values("id")]
    before_essay_questions_ids = [question["id"] for question in exam.essay_questions.all().values("id")]

    mcq_questions = MultipleChoiceQuestion.objects.filter(
        ~Q(id__in=before_mcq_questions_ids),  # ~ <==> not
    ...
    essay_questions = EssayQuestion.objects.filter(
        ~Q(id__in=before_essay_questions_ids),  # ~ <==> not
        teacher=current_teacher
    )


    return render(request, "teacher/exams/add_questions_created_before.html", {
            "exam": exam,
            "questions_number": questions_number,
...
def add_mcq_quastion_choices(request: HttpRequest, question_id: UUID) -> HttpResponse:
    question = get_object_or_404(MultipleChoiceQuestion.objects.prefetch_related("choiceanswer_set"), id=question_id)
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)
...
def add_essay_question_keyword(request: HttpRequest, question_id: UUID) -> HttpResponse:
    question = get_object_or_404(EssayQuestion.objects.prefetch_related("essayquestionkeyword_set"), id=question_id)
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)
    ...
    return render(request, "teacher/exams/add_essay_question_keywords.html", {
            "question": question,
...
@user_passes_test(teacher_user_test, "teacher")
def delete_question(request: HttpRequest, q_type: Literal["mcq", "true-false", "essay"], question_id: UUID) -> HttpResponse:
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)
...
@user_passes_test(teacher_user_test, "teacher")
def remove_question_from_exam(request: HttpRequest, exam_id: UUID, q_type: Literal["mcq", "true-false", "essay"], question_id: UUID) -> HttpResponse:
...
@user_passes_test(teacher_user_test, "teacher")
def edit_question(request: HttpRequest, q_type: Literal["mcq", "true-false", "essay"], question_id: UUID) -> HttpResponse:
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)
    ...
    return render(request, "teacher/exams/edit_question.html", {
        "form": form,
        "type": q_type,
    })
...
@user_passes_test(teacher_user_test, "teacher")
def question_details(request: HttpRequest, q_type: Literal["mcq", "true-false", "essay"], question_id: UUID) -> HttpResponse:
    current_normal_user = NormalUser.objects.get(user=request.user)
    current_teacher = Teacher.objects.get(normal_user=current_normal_user)

    if q_type == "mcq":
        question = MultipleChoiceQuestion.objects.prefetch_related("choiceanswer_set").get(id=question_id)
    elif q_type == "true-false":
        question = TrueFalseQuestion.objects.get(id=question_id)
    else:
        question = EssayQuestion.objects.prefetch_related("essayquestionkeyword_set").get(id=question_id)

    if question.teacher != current_teacher:
        return HttpResponseForbidden("<h1>403<h1/><h1>Forbidden</h1>")

    return render(request, "teacher/exams/question_details.html", {
        "question": question,
        "type": q_type,
    })
...
def ajax_add_mcq_choice(request: HttpRequest) -> JsonResponse:
    ...
    if form.is_valid():
        ...
        for choice in question.choiceanswer_set.all():  # type: ignore
            if choice.content_english == instance.content_english or choice.content_arabic == instance.content_arabic:
                return JsonResponse({
                "message": _("There is already an answer has this content in this question"),
        ...
        return JsonResponse({
            ...
            "new_choice": {
                "id": instance.id,
                "content": instance.content_english if is_english else instance.content_arabic,
...
def ajax_add_essay_keyword(request: HttpRequest) -> JsonResponse:
    ...
    if (english_keyword is None) or (arabic_keyword is None) or (english_keyword_index is None) or (arabic_keyword_index is None) or (question_id is None) or int(english_keyword_index) < 0 or int(arabic_keyword_index) < 0:
        return JsonResponse({
            "state": "error",
            "message": "Invalid request payload"
        })
    ...
    for keyword in question.essayquestionkeyword_set.all():  # type: ignore
        if keyword.english_word_index == english_keyword_index or keyword.arabic_word_index == arabic_keyword_index or keyword.content_english == english_keyword or keyword.content_arabic == arabic_keyword:
            return JsonResponse({
            "state": "error",
    ...
    return JsonResponse({
        "state": "success",
        "new_keyword_data": new_keyword_data
    })
